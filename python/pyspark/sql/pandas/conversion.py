...
class ArrowConversionMixin:
    ...
    def toRecordBatches(self, maintain_order: bool = False) -> Iterator["pa.RecordBatch"]:
        ...
        for batch in self._collect_as_arrow(
            split_batches=jconf.arrowPySparkSelfDestructEnabled(),
            maintain_order=maintain_order,
        ):
            if batch.schema.equals(arrow_schema, check_metadata=False):
                yield batch
            else:
                # If schema is different, we need to cast it to the expected schema.
                import pyarrow

                arrays = [
                    batch.column(idx).cast(type_) for idx, type_ in enumerate(arrow_schema.types)
                ]
                cast_batch = pyarrow.RecordBatch.from_arrays(arrays, schema=arrow_schema)
    ...
    def toTable(self) -> "pa.Table":
        ...
        batches = list(self.toRecordBatches(maintain_order=True))
        if len(batches) > 0:
    ...
    def _collect_as_arrow(
        ...
        if maintain_order:
            ...
            yield from (batches[i] for i in batch_order)
        else:
            import pyarrow
